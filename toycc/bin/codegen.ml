...
let rec gen_expr ctx (expr : Ast.expr) : reg * instruction list =
  ...
  | Ast.Call (fname, args) ->
    ...
    (* 计算需要通过栈传递的参数数量 (遵循RISC-V ABI) *)
    let num_stack_args = max 0 (List.length args - 8) in
    
    (* 保存需要保留的寄存器 (s0-s11) 和临时寄存器 *)
    let save_regs = ctx.saved_regs @ [S1; S2; S3; S4; S5; S6; S7; S8; S9; S10; S11] in
    let num_save_regs = List.length save_regs in
    
    (* 计算栈空间需求 (16字节对齐) *)
    let stack_args_space = num_stack_args * 4 in
    let saved_regs_space = num_save_regs * 4 in
    let stack_space = stack_args_space + saved_regs_space in
    let stack_space = (stack_space + 15) / 16 * 16  (* 16字节对齐 *) in
    ...
    let save_instrs = 
      if stack_space > 0 then
        ...
        (* 保存寄存器到栈 *)
        List.mapi (fun i reg -> 
          Sw (reg, saved_regs_space - (i + 1) * 4, Sp)
        ) save_regs
      else []
    in
    
    (* 处理参数：前8个用寄存器(a0-a7)，其余用栈 *)
    let arg_instrs =
      List.mapi
        (fun i arg ->
    ...
    let restore_instrs =
      if stack_space > 0 then
        (* 恢复寄存器 *)
        List.mapi (fun i reg -> 
          Lw (reg, saved_regs_space - (i + 1) * 4, Sp)
        ) save_regs
        @ [ Addi (Sp, Sp, stack_space) ]
      else []
    in
    
    (* 重置临时寄存器计数器，因为我们保存/恢复了它们 *)
    ctx.temp_counter <- 0;
...
let calculate_frame_size (func_def : Ast.func_def) =
  ...
  (* ra, fp + 参数 + 局部变量 + 保存的寄存器 *)
  let required_space = 8 + (num_params * 4) + (num_locals * 4) + (12 * 4) (* s0-s11 *) in
  (* 16字节对齐 *)
...
let gen_function symbol_table (func_def : Ast.func_def) : asm_item list =
  ...
  (* 恢复s寄存器 *)
  let restore_s_regs = [
    Instruction (Lw (S1, frame_size - 12, Sp));
    Instruction (Lw (S2, frame_size - 16, Sp));
    Instruction (Lw (S3, frame_size - 20, Sp));
    Instruction (Lw (S4, frame_size - 24, Sp));
    Instruction (Lw (S5, frame_size - 28, Sp));
    Instruction (Lw (S6, frame_size - 32, Sp));
    Instruction (Lw (S7, frame_size - 36, Sp));
    Instruction (Lw (S8, frame_size - 40, Sp));
    Instruction (Lw (S9, frame_size - 44, Sp));
    Instruction (Lw (S10, frame_size - 48, Sp));
    Instruction (Lw (S11, frame_size - 52, Sp));
  ] in
...
let gen_program symbol_table (program : Ast.program) =
  ...
  let header =
    [ Directive ".text"; 
      ...
      Comment "ToyC Compiler Generated Code (RISC-V ABI compliant)" ]
  in
  (* 生成所有函数 *)
  let func_asm_items =
    List.map
