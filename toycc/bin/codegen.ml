...
type codegen_context =
  ...
  ; mutable stack_offset : int    (* 局部变量区当前偏移（从fp向下增长） *)
  ; mutable break_labels : string list
  ; mutable continue_labels : string list
  ; mutable local_vars : (string * int) list  (* 变量名到fp偏移的映射 *)
  ; func_name : string
  ; total_locals : int ref
  ; frame_size : int
  }

(* 创建上下文 - 初始化局部变量偏移为fp-320（局部变量区起点） *)
let create_context _symbol_table func_name frame_size =
  { label_counter = 0;
    temp_counter = 0;
    temp_regs = [];
    stack_offset = -320;  (* 局部变量从fp-320开始向下分配 *)
    break_labels = [];
    continue_labels = [];
    local_vars = [];
    func_name = func_name;
...
(* 局部变量管理（支持参数强制偏移） *)
let add_local_var ?forced_offset ctx name =
  let offset = 
    match forced_offset with
    | Some o -> o  (* 参数使用强制计算的偏移 *)
    | None ->      (* 局部变量从fp-320向下分配 *)
        ctx.stack_offset <- ctx.stack_offset - 4;
        ctx.stack_offset
  in
  ctx.local_vars <- (name, offset) :: ctx.local_vars;
...
let rec gen_expr ctx (expr : Ast.expr) : reg * instruction list =
  ...
  | Ast.Literal(IntLit n) ->
    ...
    let instrs = if n >= -2048 && n <= 2047 then [ Li (reg, n) ]
      else let (upper, lower) = split_imm n in [ Lui (reg, upper); Addi (reg, reg, lower) ]
    in
    reg, instrs
  ...
  | Ast.Call (fname, args) ->
    ...
    (* 栈空间16字节对齐 *)
    let unaligned_space = 32 + stack_args_space + saved_regs_space in  (* 32字节影子空间 *)
    let stack_space = 
    ...
    let save_instrs = 
      if stack_space > 0 then
        ...
        [ Sw (T0, 32 + saved_regs_space - 4, Sp);  (* 影子空间之上保存寄存器 *)
          Sw (T1, 32 + saved_regs_space - 8, Sp);
          Sw (T2, 32 + saved_regs_space - 12, Sp);
          Sw (T3, 32 + saved_regs_space - 16, Sp);
          Sw (T4, 32 + saved_regs_space - 20, Sp);
          Sw (T5, 32 + saved_regs_space - 24, Sp);
          Sw (T6, 32 + saved_regs_space - 28, Sp)
        ]
      else []
    in
        ...
        (fun i arg ->
           ...
           let instrs =
             if i < 8 then
               ...
               let stack_pos = 32 + (i - 8) * 4 in  (* 影子空间后存放栈参数 *)
    ...
    let restore_instrs =
      if stack_space > 0 then
        [ Lw (T0, 32 + saved_regs_space - 4, Sp);
          Lw (T1, 32 + saved_regs_space - 8, Sp);
          Lw (T2, 32 + saved_regs_space - 12, Sp);
          Lw (T3, 32 + saved_regs_space - 16, Sp);
          Lw (T4, 32 + saved_regs_space - 20, Sp);
          Lw (T5, 32 + saved_regs_space - 24, Sp);
          Lw (T6, 32 + saved_regs_space - 28, Sp);
          Addi (Sp, Sp, stack_space)
        ]
      else []
    in
...
let gen_save_s_regs =
  [ Instruction (Sw (S0, -12, Fp));   (* s0: fp-12 *)
    ...
    Instruction (Sw (S11, -56, Fp))   (* s11: fp-56 (s寄存器区结束) *)
  ]
...
let rec gen_stmt ctx (stmt : Ast.stmt) : asm_item list =
  ...
  | Ast.Return (Some e) ->
    let e_reg, e_instrs = gen_expr ctx e in
    let all_instrs = e_instrs @ [ Mv (A0, e_reg) ] @ gen_restore_s_regs @ gen_epilogue_instrs ctx.frame_size in
    release_temp_reg ctx e_reg;
    List.map (fun i -> Instruction i) all_instrs
  
  | Ast.Return None -> 
    List.map (fun i -> Instruction i) ([ Li (A0, 0) ] @ gen_restore_s_regs @ gen_epilogue_instrs ctx.frame_size)
  ...
  | Ast.Break ->
    (match ctx.break_labels with | label :: _ -> [ Instruction (J label) ] | [] -> failwith "Break outside loop")
  
  | Ast.Continue ->
    (match ctx.continue_labels with | label :: _ -> [ Instruction (J label) ] | [] -> failwith "Continue outside loop")
  
  | Ast.Decl (name, e) ->
    let offset = add_local_var ctx name in  (* 局部变量自动分配到fp-320以下 *)
    let e_reg, e_instrs = gen_expr ctx e in
...
let calculate_frame_size (func_def : Ast.func_def) =
  ...
  let params_space = num_params * 4 in  (* 参数区（每个参数4字节） *)
  let locals_space = num_locals * 4 in  (* 局部变量区 *)
  ...
  (* 确保最小空间满足基础布局（ra+fp+s寄存器） *)
  max aligned_space (ra_fp_space + s_regs_space)
...
let gen_function symbol_table (func_def : Ast.func_def) : asm_item list =
  ...
  (* 保存S寄存器（s0-s11到fp-12至fp-56） *)
  let save_s_regs = gen_save_s_regs in
  
  (* 处理参数：从fp-60开始，每个参数占4字节 *)
  let param_instrs =
    List.mapi
      (fun i { Ast.pname = name; _ } ->
       (* 计算参数在fp中的偏移：i=0→-60, i=1→-64, ..., i=63→-316 *)
       let param_offset = -60 - (i * 4) in
       let offset = add_local_var ctx name ~forced_offset:param_offset in
       let instr =
         if i < 8 then
           (* 前8个参数：从A0-A7寄存器保存 *)
...
let gen_program symbol_table (program : Ast.program) =
  ...
  @ List.map (gen_function symbol_table) program |> List.flatten
