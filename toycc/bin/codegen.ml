...
let reg_to_string = function
  ...
  | Fp -> "fp" | S1 -> "s1" | S2 -> "s2" | S3 -> "s3" | S4 -> "s4" | S5 -> "s5"
  | S6 -> "s6" | S7 -> "s7" | S8 -> "s8" | S9 -> "s9" | S10 -> "s10" | S11 -> "s11"
  | A0 -> "a0" | A1 -> "a1" | A2 -> "a2" | A3 -> "a3" | A4 -> "a4" | A5 -> "a5" | A6 -> "a6" | A7 -> "a7"

(* 立即数拆分辅助函数（处理符号扩展） *)
let split_imm imm =
  let imm32 = imm land 0xFFFFFFFF in  (* 确保32位表示 *)
  let lower = imm32 land 0xFFF in
  let adjusted_lower =
    if (lower land 0x800) != 0 then lower - 4096  (* 符号扩展 *)
    else lower
  in
...
let create_context _symbol_table func_name =
  { label_counter = 0
  ; temp_regs = [T0; T1; T2; T3; T4; T5; T6]  (* 临时寄存器池 *)
...
let rec gen_expr ctx (expr : Ast.expr) : reg * (instruction list * asm_item list) =
  ...
  | Ast.UnOp (op, e) ->
    let e_reg, (e_instrs, e_labels) = gen_expr ctx e in
    ...
    result_reg, (e_instrs @ op_instrs, e_labels)

  | Ast.BinOp (e1, op, e2) ->
    let e1_reg, (e1_instrs, e1_labels) = gen_expr ctx e1 in
    let e2_reg, (e2_instrs, e2_labels) = gen_expr ctx e2 in
    let result_reg = get_temp_reg ctx in
    let (op_instrs, op_labels) =
      match op with
      ...
      (* 除法：带除数为零检查 *)
      | "/" -> 
          let zero_reg = get_temp_reg ctx in
          let div_zero_label = new_label ctx "div_zero" in
          let end_div_label = new_label ctx "end_div" in
          let instrs = [
              Li (zero_reg, 0);
              Beq (e2_reg, zero_reg, div_zero_label);
              Div (result_reg, e1_reg, e2_reg);
              J end_div_label
          ] in
          release_temp_reg ctx zero_reg;
          (instrs, [
              Label div_zero_label;
              Instruction (Li (result_reg, 0));  (* 除数为零返回0 *)
              Label end_div_label
          ])
      ...
      | "%" -> 
          ...
          let adjust_label = new_label ctx "adjust_rem" in
          let end_label = new_label ctx "end_rem" in
          let instrs = [
              Rem (result_reg, e1_reg, e2_reg);
              Slt (temp_reg, e1_reg, Zero);
              Beq (temp_reg, Zero, end_label);
              Bne (result_reg, Zero, adjust_label);
              J end_label
          ] in
          release_temp_reg ctx temp_reg;
          (instrs, [
              Label adjust_label;
              Instruction (Add (result_reg, result_reg, e2_reg));
              Label end_label
          ])
      ...
      | "<=" -> 
          ...
          let instrs = [
              Slt (temp, e2_reg, e1_reg);  (* temp = (b < a) 即 a > b *)
              Xori (result_reg, temp, 1)   (* 取反得到 a <= b *)
          ] in
          release_temp_reg ctx temp;
          (instrs, [])
      ...
      | ">=" -> 
          let temp = get_temp_reg ctx in
          let instrs = [
              Slt (temp, e1_reg, e2_reg);  (* temp = (a < b) *)
              Xori (result_reg, temp, 1)   (* 取反得到 a >= b *)
          ] in
          release_temp_reg ctx temp;
          (instrs, [])
      ...
      | "&&" ->
          ...
          let instrs = [
              Blt (Zero, e1_reg, false_label);  (* e1为0则结果为0 *)
              Blt (Zero, e2_reg, false_label);  (* e2为0则结果为0 *)
              Li (result_reg, 1);               (* 都非0则结果为1 *)
              J end_label
          ] in
          (instrs, [
              Label false_label;
      ...
      | "||" ->
          ...
          let instrs = [
              Bge (e1_reg, Zero, true_label);  (* e1非0则结果为1 *)
              Bge (e2_reg, Zero, true_label);  (* e2非0则结果为1 *)
              Li (result_reg, 0);              (* 都为0则结果为0 *)
              J end_label
          ] in
          (instrs, [
              Label true_label;
    ...
    let all_labels = e1_labels @ e2_labels @ op_labels in
    
    release_temp_reg ctx e1_reg;
    release_temp_reg ctx e2_reg;
    result_reg, (all_instrs, all_labels)

  | Ast.Call (fname, args) ->
    let result_reg = A0 in  (* 函数返回值存于A0 *)
    let num_args = List.length args in
    let num_stack_args = max 0 (num_args - 8) in
    let stack_args_space = num_stack_args * 4 in
    let saved_regs_space = 7 * 4 in  (* T0-T6保存空间 *)
    ...
    (* 处理函数参数 *)
    let arg_instrs, arg_labels = 
      List.mapi
        (fun i arg ->
           let arg_reg, (arg_code, arg_lbls) = gen_expr ctx arg in
           let instrs =
             if i < 8 then
               let target_reg =
                 match i with
    ...
    let all_instrs = save_instrs @ List.flatten arg_instrs @ call_instr @ restore_instrs in
    let all_labels = List.flatten arg_labels in
    result_reg, (all_instrs, all_labels)
...
(* 分离寄存器恢复指令为纯指令列表和asm_item列表两种形式 *)
let gen_restore_s_regs_instrs =
  [ Lw (S2, -16, Fp);
    Lw (S3, -20, Fp);
    Lw (S4, -24, Fp);
    Lw (S5, -28, Fp);
...
let gen_restore_s_regs = List.map (fun i -> Instruction i) gen_restore_s_regs_instrs
...
let rec gen_stmt ctx frame_size (stmt : Ast.stmt) : asm_item list =
  ...
  | Ast.ExprStmt e ->
    let reg, (instrs, labels) = gen_expr ctx e in
    release_temp_reg ctx reg;
    labels @ List.map (fun i -> Instruction i) instrs
  ...
  | Ast.Return (Some e) ->
    ...
    (* 使用纯指令列表避免模式匹配警告 *)
    let all_instrs = 
      e_instrs @ [ Mv (A0, e_reg) ] @ gen_restore_s_regs_instrs
      @ gen_epilogue_instrs frame_size
    in
    release_temp_reg ctx e_reg;
    e_labels @ List.map (fun i -> Instruction i) all_instrs
  
  | Ast.Return None -> 
    (* 使用纯指令列表避免模式匹配警告 *)
    let all_instrs = 
      [ Li (A0, 0) ] @ gen_restore_s_regs_instrs
      @ gen_epilogue_instrs frame_size
    in
    List.map (fun i -> Instruction i) all_instrs
  
  | Ast.If (cond, then_stmt, else_stmt) ->
    let cond_reg, (cond_instrs, cond_labels) = gen_expr ctx cond in
    ...
    let else_items = Option.value ~default:[] (Option.map (gen_stmt ctx frame_size) else_stmt) in
...
let calculate_frame_size (func_def : Ast.func_def) =
  let rec count_decls_in_stmt = function
    | Ast.Decl _ -> 1
    | Ast.Block stmts -> List.fold_left (fun acc s -> acc + count_decls_in_stmt s) 0 stmts
    | Ast.If (_, s1, Some s2) -> count_decls_in_stmt s1 + count_decls_in_stmt s2
  ...
  let required_space = 8 + (11 * 4) + (num_params * 4) + (num_locals * 4) in  (* ra + fp + s寄存器 + 参数 + 局部变量 *)
  let aligned = (required_space + 15) land lnot 15 in  (* 16字节对齐 *)
...
let gen_function symbol_table (func_def : Ast.func_def) : asm_item list =
  ...
  let body_items =
    func_def.body
    |> List.map (fun s -> gen_stmt ctx frame_size s)
    |> List.flatten
  in
  
  (* 检查是否有显式return，没有则添加隐式return *)
  ...
  let epilogue =
    ...
    else gen_restore_s_regs @ List.map (fun i -> Instruction i) (gen_epilogue_instrs frame_size)
  in
...
let gen_program symbol_table (program : Ast.program) =
  let header =
    [ Directive ".text"; 
      ...
      Directive ".align 2";
      Comment "ToyC Compiler Generated Code (RISC-V 32)" ]
  in
  let func_asm_items =
