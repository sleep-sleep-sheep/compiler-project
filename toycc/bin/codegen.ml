...
type codegen_context =
  ...
  ; mutable spilled_regs : spilled_reg list  (* 栈上保存的寄存器 *)
  ; mutable spill_offset : int   (* 栈上临时空间偏移，从fp开始计算（负值） *)
...
let temp_regs_list = [T0; T1; T2; T3; T4; T5; T6]  (* 可用的临时寄存器 *)
...
let spill_area_size = 128           (* 寄存器溢出区域大小，可根据需要调整 *)
...
let create_context _symbol_table func_name frame_size call_results_area_size 
  ...
  { label_counter = 0;
    temp_regs = temp_regs_list;  (* 初始化可用临时寄存器 *)
    spilled_regs = [];
    spill_offset = params_end_offset - spill_area_size - temp_regs_save_size;
...
let spill_one_register ctx =
  ...
  | [] -> failwith "No registers available to spill"
  | reg :: rest ->
    (* 更新可用寄存器列表 *)
    ctx.temp_regs <- rest;
    ...
    if ctx.spill_offset < (params_end_offset - ctx.spill_area_size - temp_regs_save_size - 4) then
      failwith "Exceeded spill area size";
    
    (* 记录溢出的寄存器及其位置 *)
    let spilled = { reg; offset } in
    ctx.spilled_regs <- spilled :: ctx.spilled_regs;
...
let rec  get_temp_reg ctx =
  ...
  | [] ->
      (* 没有可用寄存器，需要溢出一个到栈上 *)
      if ctx.spilled_regs <> [] then
        (* 如果有溢出的寄存器，恢复一个 *)
        let spilled = List.hd ctx.spilled_regs in
        ctx.spilled_regs <- List.tl ctx.spilled_regs;
        let reg = spilled.reg in
        (* 从栈中恢复寄存器的值 *)
        let restore_instr = [ Lw (reg, spilled.offset, Fp) ] in
        (reg, restore_instr)
      else
        (* 没有溢出的寄存器，需要溢出一个当前在用的 *)
        let spill_instrs = spill_one_register ctx in
        (* 递归调用，现在应该有可用寄存器了 *)
...
let rec gen_stmt ctx (stmt : Ast.stmt) : asm_item list =
  ...
  | Ast.Block stmts ->
    ...
    let old_spilled_regs = ctx.spilled_regs in
    ...
    ctx.spilled_regs <- old_spilled_regs;
