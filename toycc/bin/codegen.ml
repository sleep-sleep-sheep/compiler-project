  ...
  | Ast.Call (fname, args) ->
    ...
    let save_instrs = 
      if stack_space > 0 then
        ...
        [ Sw (T0, stack_args_space + 24, Sp);  (* T0在临时寄存器区最高地址 *)
          ...
          Sw (T6, stack_args_space, Sp)        (* T6在临时寄存器区最低地址 *)
        ]
      else []
    in
      ...
      (fun i { Ast.pname = name; _ } ->
       ...
       let instr =
         if i < 8 then
           ...
           (* 第9个及以后参数从调用者栈帧读取，使用fp访问而非sp
              在RISC-V调用约定中，调用者的栈参数位于fp+8+16的位置开始 *)
           let caller_stack_offset = 8 + 16 + ((i - 8) * 4) in  (* 修复：使用fp访问调用者栈帧 *)
           [ Instruction (Lw (T0, caller_stack_offset, Fp));  (* 从调用者栈帧加载 *)
             Instruction (Sw (T0, param_offset, Fp)) ]        (* 保存到当前函数的参数区 *)
       in
       instr)
      func_def.params
    |> List.flatten
