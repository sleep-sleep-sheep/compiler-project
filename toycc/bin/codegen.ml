...
type codegen_context =
  ...
  ; used_regs : reg list ref      (* 当前正在使用的临时寄存器 *)
  ; available_regs : reg list ref (* 可用的临时寄存器 *)
  ; spill_stack : spill_record list ref (* 溢出记录栈 *)
  ; mutable spill_offset : int    (* 寄存器溢出区域当前偏移 *)
...
let call_results_area_size = 32     (* 函数调用结果保存区32字节 *)
let spill_area_size = 128           (* 寄存器溢出区域128字节，可保存32个寄存器 *)

(* 创建上下文 - 初始化寄存器溢出管理 *)
let create_context _symbol_table func_name frame_size call_results_area_size 
    spill_area_size ret_val_offset stack_args_offset =
  { label_counter = 0;
    temp_counter = 0;
    used_regs = ref [];
    available_regs = ref all_temp_regs;  (* 初始时所有临时寄存器都可用 *)
    spill_stack = ref [];
    spill_offset = params_end_offset - 4;  (* 溢出区从参数区下方开始 *)
    stack_offset = params_end_offset - call_results_area_size - spill_area_size - temp_regs_save_size;
...
let spill_register ctx reg =
  ...
  ctx.available_regs := reg :: !(ctx.available_regs);
  
  [instr]

(* 从溢出区恢复寄存器 *)
let restore_register ctx =
  match !(ctx.spill_stack) with
  | [] -> failwith "No registers to restore from spill stack"
  | { reg; offset } :: rest ->
      ctx.spill_stack := rest;
      ctx.spill_offset <- ctx.spill_offset + 4;
      ...
      ctx.available_regs := List.filter (fun r -> r <> reg) !(ctx.available_regs);
      
      [instr]

(* 获取临时寄存器 - 如无可用则溢出一个 *)
let rec get_temp_reg ctx =
  match !(ctx.available_regs) with
  | reg :: rest ->
      (* 有可用寄存器，直接使用 *)
      ctx.available_regs := rest;
      ctx.used_regs := reg :: !(ctx.used_regs);
      reg, []
  | [] ->
      (* 无可用寄存器，溢出一个最近使用的 *)
      let reg_to_spill = List.hd !(ctx.used_regs) in
      let spill_instrs = spill_register ctx reg_to_spill in
      (* 递归调用，现在应该有可用寄存器了 *)
      let reg, instrs = get_temp_reg ctx in
      reg, spill_instrs @ instrs

(* 释放临时寄存器 - 如溢出栈有内容则恢复一个 *)
let release_temp_reg ctx reg =
  if List.mem reg !(ctx.used_regs) then begin
    ...
    (* 如果有溢出的寄存器，恢复一个 *)
    if !(ctx.spill_stack) <> [] then
      restore_register ctx
    else
      []
  end else
    []
...
let rec gen_expr ctx (expr : Ast.expr) : reg * instruction list =
  ...
  | Ast.UnOp (op, e) ->
    ...
    let instrs = e_instrs @ spill_instrs @ op_instrs @ release_instrs in
    result_reg, instrs
...
let rec gen_stmt ctx (stmt : Ast.stmt) : asm_item list =
  ...
  | Ast.Block stmts ->
    ...
    let old_available_regs = !(ctx.available_regs) in
    ...
    ctx.available_regs := old_available_regs;
  ...
  | Ast.Return (Some e) ->
    ...
    let release_instrs = release_temp_reg ctx e_reg in
    let epilogue_instrs = gen_epilogue_instrs ctx ctx.frame_size in
    let all_instrs = e_instrs @ move_instr @ release_instrs @ epilogue_instrs in
    List.map (fun i -> Instruction i) all_instrs
  ...
  | Ast.Decl (name, e) ->
    ...
    let e_reg, e_instrs = gen_expr ctx e in
    (* 将函数调用结果存储到变量中 *)
    let store_instr = [ Sw (e_reg, offset, Fp) ] in
    let release_instrs = release_temp_reg ctx e_reg in
...
let calculate_frame_size_and_offsets (func_def : Ast.func_def) =
  ...
  *)
  ...
  (frame_size, call_results_area_size, spill_area_size, ret_val_offset, stack_args_offset)

(* 函数生成 - 支持寄存器溢出 *)
let gen_function symbol_table (func_def : Ast.func_def) : asm_item list =
  let (frame_size, call_results_area_size, spill_area_size, ret_val_offset, stack_args_offset) = 
    calculate_frame_size_and_offsets func_def in
  let ctx = create_context symbol_table func_def.fname frame_size 
      call_results_area_size spill_area_size ret_val_offset stack_args_offset in
...
let gen_program symbol_table (program : Ast.program) =
  [ Directive ".text"; 
    ...
    Comment "2. 临时寄存器保存区: 28字节 (T0-T6)";
    Comment "3. 寄存器溢出区: 128字节";
